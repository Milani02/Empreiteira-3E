...
function ServiceCard({ service, index, wide }) {
  ...
  return (
    ...
    >
      <GlowCard className="h-full border border-ink/10 bg-white">
        <motion.div
          ref={ref}
          ...
          className={`group flex h-full flex-col ${wide ? "sm:flex-row" : ""}`}
        >
          <div className={`relative overflow-hidden ${wide ? "h-52 sm:h-auto sm:w-2/5" : "h-52"}`}>
            <ImageReveal
              src={service.img}
              alt={service.title}
              className="h-full"
          ...
          <div className="flex flex-1 flex-col p-6" style={{ transform: "translateZ(30px)" }}>
            ...
            <ul className={`mt-5 grid grid-cols-1 gap-1.5 border-t border-ink/10 pt-4 ${wide ? "sm:grid-cols-2" : ""}`}>
            ...
            <a
              href={waLink(`Olá! Quero um orçamento de ${service.title.toLowerCase()} com a 3M.`)}
              target="_blank"
              rel="noreferrer"
  ...
  return (
    <section id="servicos" className="bg-white py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <BentoGrid className="mt-14 lg:grid-cols-3">
          {SERVICES.map((service, i) => (
            <BentoCell key={service.code} span={i === 0 ? "full" : 1}>
              <ServiceCard service={service} index={i} wide={i === 0} />
            </BentoCell>
          ))}
        </BentoGrid>
      </div>
